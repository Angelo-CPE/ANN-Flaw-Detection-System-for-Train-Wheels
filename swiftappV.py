...
class CalibrationPage(QWidget):
    def __init__(self, parent=None):
        ...
        self.calibration_timestamps = {"700mm": None, "600mm": None}
        self.load_calibration_values()
    ...
    def on_measurement_complete(self, wheel_type):
        if wheel_type == "700mm":
            ...
            SerialReaderThread.CAL_700_RAW = distance
        else:
            self.calibration_values[wheel_type] = distance
            current_time = time.strftime("%Y-%m-%d %H:%M")
            self.calibration_timestamps[wheel_type] = current_time
            ...
            self.status_label.setText(f"{wheel_type} calibrated at {distance} mm")
            SerialReaderThread.CAL_600_RAW = distance
...
class App(QMainWindow):
    ...
    def update_status(self, status, recommendation):
        if status in ["FLAW DETECTED", "NO FLAW"] and self.current_distance != 680:
